...
class catalog
{
    ...
    { // WIP
        $sql = "SELECT p.productName, p.productPrice, p.productCategory,p.id,p. GROUP_CONCAT(t.tagName SEPARATOR ', ') AS 'tags' FROM product p, tags t,tagtoproduct tp WHERE p.id = tp.productID AND t.id = tp.TagID GROUP BY productName; ";
